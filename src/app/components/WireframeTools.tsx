  ...
  return (
    <div className="overflow-x-auto">
      <div style={{ minWidth: 270 }}>
          ...
          return (
            <div key={si} className="flex items-center mb-0.5">
              <div className="w-7 shrink-0 flex justify-end pr-1">
                <span className={`text-sm font-sans px-1 py-0.5 rounded border ${openHL ? "bg-foreground text-primary-foreground border-foreground" : "bg-muted border-border text-muted-foreground"
                  }`}>{openLetter}</span>
              </div>
                  ...
                  <div key={fi} className="flex-1 flex items-center justify-center relative"
                    ...
                    {hl ? (
                      <div className="relative z-10 w-4 h-4 rounded-full bg-foreground flex items-center justify-center">
                        <span className="text-sm font-sans font-bold text-primary-foreground">{note}</span>
                      </div>
                    ) : dot ? (
        ...
        <p className="text-sm font-sans text-muted-foreground mt-1 ml-7">
      ...
      <div className="flex gap-2">
        {notes.map((n, i) => (
          <div key={i} className="flex flex-col items-center gap-0.5">
            <span className="text-sm font-sans text-muted-foreground">{labels[i]}</span>
            <div className={`w-9 h-9 rounded-full border-2 flex items-center justify-center text-sm font-sans font-bold ${i === 0 ? "bg-foreground border-foreground text-primary-foreground" : "bg-muted border-border text-foreground"
              }`}>{n}</div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-wrap gap-1">
        {notes.map((n, i) => (
          <span key={i} className={`px-2 py-1 rounded border text-sm font-sans font-bold ${i === 0 ? "bg-foreground text-primary-foreground border-foreground" : "bg-muted text-foreground border-border"
            }`}>
            {i === 0 ? `R:${n}` : n}
          </span>
        ))}
      ...
      <div className="flex gap-1 p-0.5 bg-card border border-border rounded">
        {([
          ...
          <button
            ...
            className={`flex-1 py-1.5 rounded text-sm font-sans transition-colors ${tool === t.id ? "bg-primary text-primary-foreground" : "text-muted-foreground"
              }`}
          >
            {t.label}
          </button>
