...
function ScaleFinder({ defaultRoot = "A" }: { defaultRoot?: string }) {
...
export function WireframeTools() {
  const [tool,        setTool]        = useState<"chord"|"scale"|"circle">("chord");
  const [linkedRoot,  setLinkedRoot]  = useState("A");

  function handleCirclePick(note: string) {
    setLinkedRoot(note);
    setTool("scale");
  }
  ...
  return (
    <div className="px-4 py-4 flex flex-col gap-4">
      ...
      {/* Header */}
      <div>
        <p className="text-[10px] font-mono text-muted-foreground uppercase tracking-widest">Screen 4 of 4</p>
      ...
      {tool==="scale"  && <ScaleFinder defaultRoot={linkedRoot} />}
      ...
      {tool==="circle" && (
        <div className="border border-border rounded bg-card p-3">
          ...
          <ul className="flex flex-col gap-1.5 mt-1">
            {["Outer ring = Major keys","Inner ring = Relative minor","Adjacent = closely related keys","Tap any note → jumps to Scale Finder"].map((tip,i) => (
              <li key={i} className="flex gap-2 text-[10px] font-mono text-muted-foreground">
                <span className="text-foreground shrink-0">{i+1}.</span>{tip}
              </li>
            ))}
          </ul>
        </div>
